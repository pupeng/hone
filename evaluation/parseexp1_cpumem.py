...
def parseForCpuMem(logFileName):
    logFile = open(logFileName)
    logs = logFile.read().split('\n')
    logFile.close()
    logs.pop(len(logs) - 1)
    logs = map(lambda x : x.split(' '), logs)
    logs = filter(lambda x : len(x) >= 3, logs)
    cpu = []
    mem = []
    for log in logs:
        cpu.append(float(log[1]))
        mem.append(float(log[2]))
    print 'cpu avg:{0} min:{1} max{2} std:{3}'.format(average(cpu),
        min(cpu), max(cpu), stddev(cpu))
    print 'memory avg:{0} min:{1} max{2} std:{3}'.format(average(mem),
        min(mem), max(mem), stddev(mem))

def main():
    logFileName = sys.argv[1]
    parseForCpuMem(logFileName)
